...
export function ReportView({ report, saved = false, checkpoint = false, actions }: { report: AssessmentReport; saved?: boolean; checkpoint?: boolean; actions?: ReactNode }) {
  ...
  return (
    <div className="report-shell">
      <section className="report-hero">
        <div className="report-kicker"><span>{checkpoint ? `第 ${completedRound} 轮阶段报告` : "已保存探索报告"}</span><span>{answeredCount} 道情境 · 可信度 {confidence}%</span></div>
      ...
      <section className="report-grid">
        ...
        {report.values?.length ? (
          <article className="report-card values-card">
            <div className="card-heading"><span>04</span><div><h2>价值驱动器</h2><p>你真正愿意为之付出时间、风险与注意力的东西。</p></div></div>
        ...
        {report.regulation?.length ? (
          <article className="report-card regulation-card">
            <div className="card-heading"><span>05</span><div><h2>压力下的自动程序</h2><p>你不是“情绪稳定或不稳定”，你只是更常调用某些恢复策略。</p></div></div>
        ...
        {report.tensions?.length ? (
          <article className="report-card tensions-card">
            <div className="card-heading"><span>06</span><div><h2>你的“左右互搏”现场</h2><p>全面探索最有价值的地方，不是找最高分，而是看优势怎样彼此打架。</p></div></div>
        ...
        <article className="report-card strengths-card">
          <div className="card-heading"><span>07</span><div><h2>目前最稳的三条信号</h2><p>它们是值得验证的假设，不是刻在身份证背面的结论。</p></div></div>
